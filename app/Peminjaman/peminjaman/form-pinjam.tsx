...
import { ChevronLeft, MoreVertical, Upload, X, FileText } from 'lucide-react'
...
export default function AssetBorrowingPage() {
  ...
  const [availableAssets, setAvailableAssets] = useState<typeof employeeAssets[keyof typeof employeeAssets]>([])
  const [uploadedFiles, setUploadedFiles] = useState<UploadedFile[]>([])
  const [ownerApproval, setOwnerApproval] = useState('Pending')
  const [ownerApprovalDate, setOwnerApprovalDate] = useState('-')
  const [operatorApproval, setOperatorApproval] = useState('Pending')
  const [operatorId, setOperatorId] = useState('-')
  const [operatorApprovalDate, setOperatorApprovalDate] = useState('-')
  ...
  const handleSelectEmployee = (employee: typeof employees[0]) => {
    ...
    setAvailableAssets(employeeAssets[employee.id as keyof typeof employeeAssets])
    setSearchQuery('')
  }

  const handleAddAsset = (selectedAsset: typeof availableAssets[0]) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    console.log({ assetItems, borrowDate, returnDate, reason })
    
    // Simulate owner approval after 2 seconds
    setTimeout(() => {
      setOwnerApproval('Approved')
      setOwnerApprovalDate(new Date().toLocaleString())
      
      // Simulate operator approval after another 2 seconds
      setTimeout(() => {
        setOperatorApproval('Approved')
        setOperatorId('OP001')
        setOperatorApprovalDate(new Date().toLocaleString())
      }, 2000)
    }, 2000)
  }
      ...
      <div className="border-b bg-white px-4 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-semibold">Asset Borrowing Request</h1>
              <div className="flex items-center gap-3 text-sm">
                <span className="text-gray-500">ID: 123</span>
                <span className="rounded bg-yellow-100 px-2 py-0.5 text-yellow-700">DRAFT</span>
        ...
        <div className="w-80 border-l bg-white px-4 py-6">
          <div className="space-y-6">
            <div>
              ...
              <span className="mt-1 inline-block rounded bg-yellow-100 px-2 py-0.5 text-sm text-yellow-700">Draft</span>
            </div>
            ...
            <div>
              ...
              <div className="mt-2 flex items-center gap-2">
                <div>
                  <p className="text-sm font-medium">Joko</p>
            ...
            <div>
              ...
              <p className="mt-1 text-sm">Engineering</p>
            </div>

            <div>
              <h2 className="font-semibold">OWNER APPROVAL</h2>
              <p className="mt-1 text-sm">{ownerApproval}</p>
            </div>

            <div>
              <h2 className="font-semibold">OWNER APPROVAL DATE</h2>
              <p className="mt-1 text-sm">{ownerApprovalDate}</p>
            </div>

            <div>
              <h2 className="font-semibold">OPERATOR APPROVAL</h2>
              <p className="mt-1 text-sm">{operatorApproval}</p>
            </div>

            <div>
              <h2 className="font-semibold">OPERATOR ID</h2>
              <p className="mt-1 text-sm">{operatorId}</p>
            </div>

            <div>
              <h2 className="font-semibold">OPERATOR APPROVAL DATE</h2>
              <p className="mt-1 text-sm">{operatorApprovalDate}</p>
            </div>
          </div>

          <div className="mt-8">
            <Button className="w-full" onClick={handleSubmit}>Submit Borrowing Request</Button>
          </div>
        </div>
      </div>
    </div>
